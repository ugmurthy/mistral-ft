...
import { createUserSession, loginUser , createLoginSession, addAuthToken, getUserId} from "../module/session/session.server"
...
export async function action(args: ActionFunctionArgs) {
  ...
  if (result.success) {
    ...
    return redirect('/', { headers: await createUserSession(user) });
  }
      ...
      {data?.passwordError && <div className="font-thin text-red-700">{data.passwordError}</div>}
        ...
        <div className="form-control">
          <label className="label cursor-pointer">
            ...
            <input name="rememberMe" type="checkbox" defaultChecked className="checkbox checkbox-sm md:checkboc-md" />
          </label>
        </div>
        <div className="flex flex-row justify-between  ">
        <button className="btn btn-neutral btn-sm " type="submit">Login</button>
        <Link to="/signup" className="text-sm  underline cursor-pointer place-content-center">Sign Up</Link>
        </div>
        </div>
      </Form>
    </div>
